...
class FuzzyRule(object):
    ...
    def __repr__(self):
        return 'Inputs: {0} \nResults: {1} \nCompiled statement: {2}'.format(len(self._inputs), len(self.result),
                                                                             self._execStatement)
    ...
    def evaluate_rule(self, invals):
        ...
        try:
            return eval(self._execStatement, {'self': self, 'inVals': invals, 'FuzzyRule': FuzzyRule})
        except NameError:
            print(invals)
            raise

    def build_rule_from_string(self, instr):
        """ Take grammer and convert to string of logic that can be executed by Python.
        ...
        impstr = 'self.result.get_implication("{1}",{2})'
        ...
        self._inpQueryTable = {}
        condition = self._parse_tokens(tokens)
    ...
    def _parse_tokens(self, tokens):
        ...
        """
        # format strings
        assignstr = 'self._inputs["{0}"].truth_for_statement(inVals["{0}"],"{1}")'
        ...
        while i < len(tokens):
            if tokens[i] == '(':
                ...
                tokens[start:end+1] = ['(' + self._parse_tokens(tokens[start + 1:end]) + ')']
            i += 1
        ...
        reservedlist = ['is', 'not', 'then', ','] + list(booldict.keys()) + list(funcdict.keys())
        ...
        while i < len(tokens):
            if tokens[i].lower() == 'is':
                ...
                self._inpQueryTable[tokens[i - 1]] = tokens[i + 1]
    ...
    def xorop(lhs, rhs):
        ...
        andop = self.__class__.andop
        orop = self.__class__.orop
        notop = self.__class__.notop
...
class FuzzyInput(object):
    ...
    def truthstrlist(self):
        ...
        """
        return [x for x in self._truthCurves]
...
class FuzzyLogicSet(object):
    ...
    def import_rules(self, rules=None):
        ...
        Args:
            rules (list, optional): A list of string to be evaluated as rules. If None, use textRules attribute
              instead.
